...
export class AuthService {
  ...
  async residencyAdminLogin({ username, password }: AdminPayloadDto) {
    try {
      const findUser: any = await this.prisma.residencyAdmin.findFirst({
        where: {
          email: username,
        },
      });
      ...
      return {
        message: 'Residency admin validated',
        admin: findUser,
        token,
        ok: true,
      };
    } catch (error) {
      console.log('Error login residency admin ', error);
      // Handle different types of errors
      if (error instanceof UnauthorizedException) {
        throw error; // Will automatically return 401 and the provided message
      }
